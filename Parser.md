...
- parser — не единственный клиент `Game API`: тем же shared API пользуются UI, scripts, игровая логика и actor-aware runtime исполнители вроде Puppet Master.
...
Отдельно `context.scene.recentTurns` даёт LLM короткую историю текущего визита в сцену: последние 8 команд игрока и player-facing ответов парсера. Ответ хранится в укороченном виде до 85 символов. История runtime-only, не сериализуется в scene JSON и очищается при входе в сцену после ухода из неё, чтобы повторный визит начинался без старых turns.
...
- тем же API могут пользоваться UI, scripts, игровая логика и actor-aware runtime исполнители;
