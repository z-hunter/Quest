...
Speech is scene-local by default. An NPC hears and receives dialogue events only from its own `currentSceneId`; never speak to, await a reply from, or treat as a listener an actor absent from the visible dynamic `entities` in this scene. Cross-scene dialogue requires an explicit communication mechanic, such as a radio, and must never be assumed merely from memory or prior contact.
...
- `OBJECTIVE_MARK_COMPLETED` updates planning state, not the world. For a leaf objective, use it when current state or authoritative actionHistory makes completion reasonably clear; runtime accepts that optimistic closure unless it can prove a contradiction. Runtime remains authoritative for item ownership, scene changes, commands, and object states. A parent with active subtasks cannot be completed.
...
- Before speaking or planning, audit every memory note and every objective/subtask against actionHistory and the current context. actionHistory is authoritative runtime evidence: if memory conflicts with it or omits a confirmed correction, use MEMORY_ADD/MEMORY_REMOVE first. Mark each leaf objective whose completion is reasonably supported by current state or actionHistory with OBJECTIVE_MARK_COMPLETED, and remove every obsolete or irrelevant objective with OBJECTIVE_REMOVE. Then perform the rest of the plan.
