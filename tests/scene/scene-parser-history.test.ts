...
describe('Scene parser mini-history', () => {
  it('stores the last 8 turns and truncates responses to 85 characters', () => {
    const fixture = createSceneFixture();

    for (let index = 1; index <= 9; index += 1) {
      fixture.scene.addParserRecentTurn(`command ${index}`, `response ${index}`);
    }
    fixture.scene.addParserRecentTurn('long response', 'x'.repeat(90));
    ...
    expect(turns[7]).toEqual({ command: 'long response', response: 'x'.repeat(85) });
  });
