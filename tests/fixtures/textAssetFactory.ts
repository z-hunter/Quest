...
const DEFAULT_SERVICE_TEXT: Record<string, string | string[]> = {
  ...
  'engine.observed_give': '[ {actor} gives {item} to {target} ]',
  'engine.observed_give_failed':
    '[ {actor} tries to give {item} to {target}, but fails: {reason} ]',
  ...
  'parser.give_prompt': 'Give what to whom? (Format: GIVE ITEM TO ACTOR)',
  'parser.give_which_item': 'Which item do you want to give: {options}?',
  'parser.give_which_target': 'Whom do you want to give it to: {options}?',
  'parser.give_target_not_found': "You don't see that person here.",
  'parser.give_self': "You can't give something to yourself.",
  'parser.give_success': 'You give the {item} to {target}.',
  'parser.give_received_intro': '{actor} gives you {item}.',
...
const DEFAULT_PARSER_TRAINING: ParserTrainingAsset = {
  ...
  give: ['give key to guard', 'hand note to linda'],
