...
export class EditorUI {
  ...
  destroy(): void {
    if (this.editor.boundKeyHandler)
      document.removeEventListener('keydown', this.editor.boundKeyHandler, { capture: true });

    if (this.editor.boundMouseDownHandler)
      this.editor.game.canvas.removeEventListener('mousedown', this.editor.boundMouseDownHandler);
    if (this.editor.boundMouseMoveHandler)
      window.removeEventListener('mousemove', this.editor.boundMouseMoveHandler);
    if (this.editor.boundMouseUpHandler)
      window.removeEventListener('mouseup', this.editor.boundMouseUpHandler);
    if (this.editor.boundWheelHandler)
      this.editor.game.canvas.removeEventListener('wheel', this.editor.boundWheelHandler);
  ...
  setupListeners(): void {
    ...
    this.editor.game.canvas.addEventListener('wheel', this.editor.boundWheelHandler, {
      passive: false,
    });
