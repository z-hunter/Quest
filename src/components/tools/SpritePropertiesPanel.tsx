  ...
  return (
    ...
    >
      <div className="editor-header">
        ...
        <button className="e-btn" onClick={() => toggleSpriteEditor(false)}>
          X
        </button>
      </div>
        ...
        <div className="e-row" style={{ marginTop: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <label className="e-label" style={{ marginRight: '5px', marginBottom: 0 }}>
                Speed(ms)
              </label>
              <input
                type="number"
                ...
                onChange={(e) =>
                  handleEditorChange('previewSpeed', parseInt(e.target.value) || 100)
                }
              />
            </div>
          </div>
        ...
        <div className="e-row" style={{ borderTop: '1px solid #444', paddingTop: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '5px', marginBottom: '10px' }}>
            <button
              ...
              style={{
                background: '#000',
                border: spriteEditor.previewBg === 'black' ? '2px solid #fff' : '1px solid #555',
                width: '30px',
                height: '20px',
              }}
              onClick={() => handleEditorChange('previewBg', 'black')}
            ></button>
            <button
              className="e-btn"
              style={{
                background: '#999',
                border: spriteEditor.previewBg === 'checker' ? '2px solid #fff' : '1px solid #555',
                width: '30px',
                height: '20px',
              }}
              onClick={() => handleEditorChange('previewBg', 'checker')}
            ></button>
            <button
              className="e-btn"
              style={{
                background: '#ff00ff',
                border: spriteEditor.previewBg === 'pink' ? '2px solid #fff' : '1px solid #555',
                width: '30px',
                height: '20px',
              }}
              onClick={() => handleEditorChange('previewBg', 'pink')}
            ></button>

            <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
              <label
                className="e-label"
                style={{ display: 'flex', alignItems: 'center', margin: 0, cursor: 'pointer' }}
              >
                <input
                  type="checkbox"
                  checked={spriteEditor.showRulers}
                  onChange={(e) => handleEditorChange('showRulers', e.target.checked)}
