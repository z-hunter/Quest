...
## NPC cognition fields

NPC object Text Assets may provide initial cognition. `memory` is a string or flat string array, with one factual memory per entry. `objectives` is a string, flat string array, or nested `{ "text": "...", "subtasks": [...] }` tree. Runtime task IDs are generated by the engine.
